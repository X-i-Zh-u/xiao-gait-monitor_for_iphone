...
class CsvRecorder {
  ...
  async stop() {
    ...
    const blob = new Blob(["\ufeff", header, ...chunks.map(chunk => chunk.text)], {type: "text/csv;charset=utf-8"});
    if (this.downloadUrl) URL.revokeObjectURL(this.downloadUrl);
    this.downloadUrl = URL.createObjectURL(blob);
    const link = $("record-download");
    link.href = this.downloadUrl;
    link.download = this.session + ".csv";
    link.hidden = false;
  }
}
...
function updateRecordingUi() {
  ...
  } else if (recorder.downloadUrl) {
    $("record-title").textContent = `CSV已生成 · ${recorder.rows} 行`;
    $("record-detail").textContent = "点击“下载CSV”并保存到iPhone文件中";
  } else {
...
$("record-stop").addEventListener("click", async () => {
  try {
    await recorder.stop();
    notice(`记录完成，共 ${recorder.rows} 行。请点击“下载CSV”。`);
  } catch (error) {
